...
I’m an engineer (BSEE, MSEE, University of florida) working at the intersection of **reinforcement learning, robotics-oriented simulation, and agentic AI systems** — with a long-term focus on embodied intelligence and real-world control.

This GitHub reflects a structured transition:
from classical software and control engineering → modern AI agents → reinforcement learning → physics-based robotics simulation.
...
- **Reinforcement Learning Foundations → Locomotion → Robotics Simulation**
...
### 🔹 RL → Robotics Learning Stack
A staged learning path documenting the transition from tabular RL to continuous locomotion and physics-based control.

- **Stage 1 — RL Foundations**: tabular methods and control intuition  
- **Stage 2 — Neural Policy RL**: DQN, policy gradients, actor–critic, PPO  
- **Stage 3 — Continuous Control & Locomotion**: PPO in physics environments (Pendulum → BipedalWalker → MuJoCo)

📌 See the `rl-to-robotics-learning-stack` repo and linked stage repos.
...
as a **control problem**, grounded in dynamics, feedback, and behavior over time — not just model outputs.
...
> “The biggest breakthroughs in AI have come from methods that scale with computation.”  
...
- Isaac Gym / Isaac Lab–style environments
- More explicit reward design, diagnostics, and policy evaluation
- Deeper focus on robustness, efficiency, and stability

The goal is not just to train agents — but to **understand what they learn and why**.
